...
function createDeliveryJob(storage, world) {
  ...
  function save(){
    try { storage.setItem(key, JSON.stringify({pendingOnlineHome,bankBalance,coins,completed,ownedHomes,homeId,ownedOutfits,outfitId})); storageAvailable=true; }
    catch { storageAvailable=false; }
  }
  ...
  function spend(amount){
    ...
    try{storage.setItem(key,JSON.stringify({pendingOnlineHome,bankBalance,coins:coins-amount,completed,ownedHomes,homeId,ownedOutfits,outfitId}));}catch{storageAvailable=false;return false;}
  ...
  function bankTransfer(amount){
    ...
    try{storage.setItem(key,JSON.stringify({pendingOnlineHome,bankBalance:bankBalance+amount,coins:coins-amount,completed,ownedHomes,homeId,ownedOutfits,outfitId}));}catch{storageAvailable=false;return false;}
    coins-=amount;bankBalance+=amount;storageAvailable=true;return true;
  }
  function reserveOnlineHome(id,request){
    const home=world.homes.find(h=>h.id===id);if(!home||pendingOnlineHome||coins<home.price)return false;
    const next={id,request,amount:home.price};
    try{storage.setItem(key,JSON.stringify({bankBalance,coins:coins-home.price,completed,ownedHomes,homeId,ownedOutfits,outfitId,pendingOnlineHome:next}));}catch{storageAvailable=false;return false;}
    coins-=home.price;pendingOnlineHome=next;return true;
  }
  function finishOnlineHome(result){
    if(!pendingOnlineHome||result.request!==pendingOnlineHome.request)return false;
    const balance=coins+(result.ok?0:pendingOnlineHome.amount);
    try{storage.setItem(key,JSON.stringify({bankBalance,coins:balance,completed,ownedHomes,homeId,ownedOutfits,outfitId,pendingOnlineHome:null}));}catch{storageAvailable=false;return false;}
    coins=balance;pendingOnlineHome=null;return true;
  }
  function purchaseHome(id){
    ...
    const next={pendingOnlineHome,bankBalance,coins:coins-house.price,completed,ownedHomes:[...ownedHomes,id],homeId:homeId||id,ownedOutfits,outfitId};
  ...
  function setHome(id){
    ...
    try{storage.setItem(key,JSON.stringify({pendingOnlineHome,bankBalance,coins,completed,ownedHomes,homeId:id,ownedOutfits,outfitId}));}
  ...
  function purchaseOutfit(id){
    ...
    const next={pendingOnlineHome,bankBalance,coins:coins-outfit.price,completed,ownedHomes,homeId,ownedOutfits:[...ownedOutfits,id],outfitId:id};
  ...
  function equipOutfit(id){
    ...
    try{storage.setItem(key,JSON.stringify({pendingOnlineHome,bankBalance,coins,completed,ownedHomes,homeId,ownedOutfits,outfitId:id}));}catch{storageAvailable=false;return false;}
