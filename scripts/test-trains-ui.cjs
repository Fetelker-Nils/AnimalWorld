...
(async()=>{const server=createServer(path.resolve(__dirname,'../dist'));await new Promise(r=>server.listen(0,'127.0.0.1',r));const browser=await chromium.launch({headless:true});try{
 ...
 await p.evaluate(()=>{const t=window.__animalTest,b=t.state().trains[0],c=Math.cos(b.heading),s=Math.sin(b.heading);t.visit(b.x+c*2.6*1.6-s*2.5*1.2,b.y+s*2.6*1.6+c*2.5*1.2,b.heading-Math.PI/2);window.dispatchEvent(new KeyboardEvent('keydown',{key:'w'}));t.advance(.5);window.dispatchEvent(new KeyboardEvent('keyup',{key:'w'}));});
 ...
 await p.evaluate(()=>window.__animalTest.advance(45));const riding=await p.evaluate(()=>window.__animalTest.state());assert(riding.trains[0].speed>0);assert(Math.hypot(riding.x-riding.trains[0].x,riding.y-riding.trains[0].y)<9,'Ride follows train');
