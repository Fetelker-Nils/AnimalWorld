...
(async()=>{
 ...
 try{
  ...
  await page.routeWebSocket('**/play',ws=>{socket=ws;ws.onMessage(raw=>{const m=JSON.parse(raw);if(m.type==='identity')ws.send(JSON.stringify({type:'properties',sold:['village'],mine:['village']}));if(m.type==='sleep'){requests.push(m.sleeping);ws.send(JSON.stringify({type:'sleep',sleepers:m.sleeping?1:0,total:2}));}});ws.send(JSON.stringify({type:'welcome',id:'tester',minutes:1260,players:[]}));});
