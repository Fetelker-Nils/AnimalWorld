...
(async()=>{
  ...
  try {
    ...
    for(const [file,type] of [['robots.txt','text/plain'],['sitemap.xml','application/xml'],['spielinfo.html','text/html']]){
    ...
    assert(html.includes('rel="canonical"'));assert(html.includes('name="description"'));assert(html.includes('href="spielinfo.html"'));
