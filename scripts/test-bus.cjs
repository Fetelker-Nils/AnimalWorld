...
const packet=simulation.snapshot();replica.accept({...packet,buses:[...packet.buses,packet.buses[0]]});assert.equal(replica.buses.length,6,'Snapshots cannot duplicate a bus');
...
console.log('PASS all four lines, unique bus replicas and non-explosive car/bus collision');
