...
vm.runInContext(fs.readFileSync(path.join(root,'game.js'),'utf8').replace('  function frame(now)','  globalThis.busTest={life,Island,mauz,keys,step,carryBus,busLocal,busPoint,busWalkable,setMode};\n  function frame(now)'),sandbox);
const {life,Island,mauz,keys,step,carryBus,busLocal,busPoint,busWalkable,setMode}=sandbox.busTest;
...
for(const name of ['Nordstadt','Stadtzentrum','Dorf','Hafenstrasse','Flugplatz'])assert(visited.has(name),'Visit '+name);
