...
(async()=>{
 ...
 try{
  ...
  for(const id of ['helicopter','plane']){
   ...
   await page.keyboard.down('q');if(id==='plane')await page.keyboard.down('w');await page.evaluate(()=>window.__animalTest.advance(2.5));await page.keyboard.up('q');await page.keyboard.up('w');
  ...
  for(const id of ['plane','helicopter']){
    await page.evaluate(()=>{window.__animalTest.visit(225,220);document.querySelector('#interact').click();});await page.click('[data-car="'+id+'"]');await page.evaluate(()=>{const p=window.__animalTest.state().carPosition;window.__animalTest.visit(p.x,p.y);});await page.keyboard.press('f');
    await page.evaluate(()=>{window.__animalTest.airCrashSetup();for(let i=0;i<300&&window.__animalTest.state().car;i++)window.__animalTest.advance(1/60);});
    const crash=await page.evaluate(()=>window.__animalTest.state());assert(!crash.car&&crash.explosionParticles>0,id+' explodes on impact');
  }
  await page.evaluate(()=>{window.__animalTest.visit(550,0);document.querySelector('#interact').click();});await page.click('[data-car="boat"]');
  await page.evaluate(()=>window.__animalTest.visit(563,2.6));await page.keyboard.press('f');await page.keyboard.down('w');await page.evaluate(()=>{for(let i=0;i<900&&window.__animalTest.state().car;i++)window.__animalTest.advance(1/60);});await page.keyboard.up('w');
  const boat=await page.evaluate(()=>window.__animalTest.state());assert.equal(boat.car,null,'Fast collision destroys boat');assert(!boat.driving);assert(boat.explosionParticles>0,'Visible kaboom');
  ...
  for(const id of ['helicopter','plane','boat']){
    ...
    if(id!=='boat')await page.keyboard.down('q');if(id!=='helicopter')await page.keyboard.down('w');await page.evaluate(()=>window.__animalTest.advance(2.5));await page.keyboard.up('q');await page.keyboard.up('w');
