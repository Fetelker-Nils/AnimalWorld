...
export class World extends DurableObject {
  ...
  async webSocketMessage(ws, message) {
    ...
    if(data.type==='bus-seat'){
      const b=[...this.life.buses,...this.life.trains,...this.life.planes].find(b=>b.id===p.busId&&b.id===data.busId);
    ...
    const passengerFlight=this.life.planes.find(b=>b.id===p.busId&&b.id===data.busId&&Math.hypot(b.x-data.x,b.y-data.y)<40);
    if(![data.x,data.y,data.heading,data.jump].every(Number.isFinite)||!globalThis.AnimalIsland.inBounds(data.x,data.y)||Math.abs(data.heading)>1e6||data.jump<0||data.jump>(passengerFlight?passengerFlight.z+2.5:6))return;
    ...
    if(v&&['compact','roadster','pickup','plane','helicopter','boat'].includes(v.model)&&[v.x,v.y,v.heading,v.speed].every(Number.isFinite)&&globalThis.AnimalIsland.inBounds(v.x,v.y)&&Math.abs(v.speed)<=65&&Number.isFinite(v.z??0)&&(v.z??0)>=0&&(v.z??0)<=100&&Math.abs(v.heading)<1e6){p.vehicle={model:v.model,x:v.x,y:v.y,heading:v.heading,speed:v.speed,z:v.z??0};}
    ...
    p.busId=[...this.life.buses,...this.life.trains,...this.life.planes].find(b=>b.id===data.busId&&Math.hypot(b.x-p.x,b.y-p.y)<(b.kind==='plane'?32:b.kind==='train'?24:7))?.id||null;
