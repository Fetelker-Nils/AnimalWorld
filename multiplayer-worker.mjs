...
export class World extends DurableObject {
  constructor(ctx, env) {
    ...
    this.clockOffset=0;ctx.blockConcurrencyWhile(async()=>{this.clockOffset=await ctx.storage.get('clockOffset')||0;});
  ...
  async webSocketMessage(ws, message) {
    ...
    if(data.type==='sleep'){
      ...
      if(data.sleeping===true&&(!(hour>=20||hour<6)||!p.room.startsWith('home:')||Math.hypot(p.x+8,p.y+8)>4.8||p.car||p.riding)){
    ...
    if(p.sleeping&&(!p.room.startsWith('home:')||Math.hypot(p.x+8,p.y+8)>4.8)){p.sleeping=false;await this.checkSleep();}
