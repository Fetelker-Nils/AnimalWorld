...
Lokale Angebote: Alle acht Pfotenland-Orte sowie Dorf und Perleninsel haben jeweils zwei zusaetzliche Jobs mit eigenen Arbeitsgebaeuden, unabhaengigen Auftragsrunden und den interaktiven Regeln des jeweiligen Jobtyps. Jede Bahnhofslinie 8 bis 16 startet mit zwei raeumlich getrennten Bussen. Pruefung: `node scripts/test-local-jobs-buses.cjs`. Fuer die Online-Flotte muss neben dem Frontend auch der Worker mit der aktualisierten world.js veroeffentlicht werden.

Weltausbau: Pfotenland-Radius 6700 m, Weltgrenze 16000 m, mehr als fuenffache Landflaeche gegenueber dem Stand mit Radius 2850 m. 28 Orte, 24 Bahnlinien, 25 Zuege mit 75 Wagen und 67 Busse. Neue Orte haben jeweils zwei Jobs und eine Bahnhofslinie. Umsteigebahnsteige haben erweiterte Daecher. Test: `node scripts/test-world-expansion.cjs`. Frontend und Online-Worker muessen zusammen aktualisiert werden.

Die 20 neuen Orte verwenden feste, einzeln gesetzte Koordinaten in `expansionPositions` statt eines Rasters. `linkPairs` legt ihre Bahnverbindungen dauerhaft fest; Bahn- und Strassenkorridore sowie lokale Buslinien folgen diesen Standorten. Es gibt keine zufaellige Ortsverteilung pro Welt oder Spielstart.

Ansagen: `assets/sound/manifest.json` umfasst alle 282 benoetigten MP3s fuer Buslinien 1-36, Regionalzuege R1-R24, aktuelle Haltestellen und Endstationen. Fehlende Dateien erzeugt `python scripts/generate-bus-audio.py` (Entwicklungswerkzeug, edge-tts 7.2.8); das Spiel verwendet ausschliesslich lokale MP3s. Vollstaendige Browser-Dekodierpruefung: `node scripts/test-all-announcements.cjs` nach dem Build.

Mauz Hauptbahnhof: grosse begehbare Halle (100 x 42 m) mit Dach, Saeulen, Sitzbaenken, Automaten und Fahrplan-Zugang. Acht neue Inselverbindungen RE1/2, IC1/2, ICE1/2 und UE1/2 bedienen zwei Inseln mit je drei Staedten und drei Doerfern. RE: 216 km/h, alle Inselorte; IC: 270 km/h, Staedte; ICE: 324 km/h, ausgewaehlte Hauptstationen; UE: 396 km/h, direkte Verbindung zum Inselhauptort. Spielwerte, keine realen Fahrplaene. 24 weitere Busse auf 12 neuen lokalen Linien; alle neuen Ansagen liegen als MP3 vor. Pruefung: `node scripts/test-express-islands.cjs`.
