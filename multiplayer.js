function createMultiplayer(onStatus, onClock, onWorld=()=>{}, onCrash=()=>{}, onSession=()=>{}) {
  ...
  const players=new Map();let rideOwner=null;let sold=[],mine=[],identity=null;
  function report(next){status=next;onStatus(status,players.size+(id?1:0));}
  function stop(){generation++;clearInterval(timer);clearTimeout(timeout);timer=timeout=null;if(socket){socket.onclose=socket.onerror=socket.onmessage=null;socket.close();}socket=null;id=null;rideOwner=null;players.clear();sold=[];mine=[];report('offline');}
    ...
    return new Promise((resolve,reject)=>{
      ...
      socket.onmessage=event=>{
        ...
        if(data.type==='welcome'){
          ...
          joined=true;clearTimeout(timeout);onClock(data.minutes);report('online');if(identity)socket.send(JSON.stringify({type:'identity',token:identity}));resolve();
