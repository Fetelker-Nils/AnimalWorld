function createOnlinePanel(send,stopMoving,notify=()=>{}){
  ...
  let unread=0,lastNotice=-Infinity;
  const badge=document.querySelector('#chat-unread');
  function updateUnread(){
    badge.hidden=unread===0;badge.textContent=unread>99?'99+':String(unread);
    badge.setAttribute('aria-label',unread+' neue Nachrichten');
    const toggle=document.querySelector('#online-toggle'),collapsed=document.querySelector('#online-body').hidden;
    const label=(collapsed?'Chat aufklappen':'Chat einklappen')+(unread?' - '+unread+' neue Nachrichten':'');
  ...
  document.querySelector('#online-toggle').onclick=()=>{const body=document.querySelector('#online-body');body.hidden=!body.hidden;const toggle=document.querySelector('#online-toggle');toggle.setAttribute('aria-expanded',String(!body.hidden));const label=body.hidden?'Chat aufklappen':'Chat einklappen';toggle.setAttribute('aria-label',label);toggle.title=label;if(body.hidden)input.blur();else{log.scrollTop=log.scrollHeight;unread=0;}updateUnread();};
  return {
    show(visible){root.hidden=!visible;if(!visible)input.blur();},
    status(status,id){self=id;connected=status==='online';input.disabled=!connected;submit.disabled=!connected||pending;if(status==='offline'){unread=0;updateUnread();messages=[];players=[];release();list.replaceChildren();renderMessages();feedback.textContent='';}else if(!connected){release();feedback.textContent='Chat ist nicht verbunden.';}},
    receive(data){
      ...
      if(data.type==='chat-history'){unread=0;updateUnread();messages=data.messages.slice(-100);renderMessages();}
      if(data.type==='chat-message'){if(!messages.some(m=>m.id===data.message.id)){if(data.message.playerId!==self&&document.querySelector('#online-body').hidden){unread++;updateUnread();const now=Date.now();if(now-lastNotice>=800){lastNotice=now;notify();}}const bottom=log.scrollHeight-log.scrollTop-log.clientHeight<35;if(!messages.length)log.replaceChildren();messages.push(data.message);messages=messages.slice(-100);log.append(line(data.message));while(log.children.length>100)log.firstElementChild.remove();if(bottom)log.scrollTop=log.scrollHeight;}if(data.message.playerId===self){release();input.value='';feedback.textContent='Gesendet';}}
