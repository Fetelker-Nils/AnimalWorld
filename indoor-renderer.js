...
function createIndoorRenderer({transparent=false}={}){
  ...
  const textured=gl.getUniformLocation(program,'textured'),opacity=gl.getUniformLocation(program,'opacity');
  return {surface,dispose(){gl.deleteTexture(texture);gl.deleteBuffer(buffer);gl.deleteProgram(program);gl.getExtension('WEBGL_lose_context')?.loseContext();surface.width=surface.height=1;},render({width,height,scale,cx,cy,point,boxes,floors,sprite,catDepth,extras=[],mesh=[],occluders=[],composite=true,farDistance=150,labels=[]}){
    ...
    gl.depthMask(true);gl.uniform1f(opacity,1);gl.viewport(0,0,surface.width,surface.height);gl.colorMask(true,true,true,true);gl.clearColor(transparent?0:.87,transparent?0:.86,transparent?0:.81,transparent?0:1);gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);
    ...
    for(const f of mesh.filter(f=>(f.opacity??1)>=1)){const rgb=[1,3,5].map(i=>parseInt(f.color.slice(i,i+2),16)/255);for(let i=1;i<f.points.length-1;i++)for(const v of [f.points[0],f.points[i],f.points[i+1]]){const p=point(...v);vertices.push(2*scale/width*p.u+(2*cx/width-1)*p.depth,2*scale/height*p.v+(1-2*cy/height)*p.depth,A*p.depth+B,p.depth,...rgb,0,0);}}
    ...
    const glass=mesh.filter(f=>(f.opacity??1)<1).map(f=>({...f,depth:f.points.reduce((sum,v)=>sum+point(...v).depth,0)/f.points.length})).sort((a,b)=>b.depth-a.depth);
    if(glass.length){
      gl.uniform1i(textured,0);gl.enable(gl.BLEND);gl.blendFuncSeparate(gl.SRC_ALPHA,gl.ONE_MINUS_SRC_ALPHA,gl.ONE,gl.ONE_MINUS_SRC_ALPHA);gl.depthMask(false);
      for(const f of glass){vertices.length=0;const rgb=[1,3,5].map(i=>parseInt(f.color.slice(i,i+2),16)/255);
        for(let i=1;i<f.points.length-1;i++)for(const v of [f.points[0],f.points[i],f.points[i+1]]){const p=point(...v);vertices.push(2*scale/width*p.u+(2*cx/width-1)*p.depth,2*scale/height*p.v+(1-2*cy/height)*p.depth,A*p.depth+B,p.depth,...rgb,0,0);}
        gl.uniform1f(opacity,f.opacity);gl.bufferData(gl.ARRAY_BUFFER,new Float32Array(vertices),gl.DYNAMIC_DRAW);gl.drawArrays(gl.TRIANGLES,0,vertices.length/9);
      }
      gl.depthMask(true);gl.uniform1f(opacity,1);
    }
    if(!composite)return;
    gl.uniform1i(textured,1);gl.enable(gl.BLEND);gl.blendFunc(gl.SRC_ALPHA,gl.ONE_MINUS_SRC_ALPHA);
